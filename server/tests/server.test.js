...
describe('PATCH /todos/:id', ()=>{
  ...
  it('should not update todo the belongs to another user', (done)=>{
    // grab id of first item
    ...
    request(app)
      ...
      // assert that we get 200
      .expect(404)
      // assert tht text is not changed, competed is true,
      .expect((res)=>{
...
describe('DELETE /users/me/token', () => {
  ...
  it('should remove auth token on logout', (done) => {
    ...
    request(app)
      .delete('/users/me/token')
      // set x-auth equal ro token
      .set('x-auth', users[0].tokens[0].token)
    // 200
      .expect(200)
    // find user in database, verify that token array has length to be zero
      .end( (err, res) => {
        if(err){
          return done(err);
        }
